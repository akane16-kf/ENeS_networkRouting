...
class PhysarumSolver(object):
    ...
    def __init__(self, G, nodes, edges, edges_added, nodesNum, source, sink, route_border, route_candidate_border, totalFlow, repeatNum, delta_t, mu):
        self.G = G
        self.nodes = nodes
        self.edges = edges
    ...
    def SetA(self, L, D):
        ...
        for e in D:
            ...
            if e not in blacklist_e:
                self.G.edges[e]['C'] = D[e]/L[e]
                blacklist_e.append(revers_e)
            else:
                self.G.edges[e]['C'] = D[revers_e]/L[revers_e]
    ...
    def setL(self):
        ...
        for e in nx.edges(self.G):
            ...
            if e in self.edges_added:
                #L[e] = sys.float_info.max
                L[e] = ((self.nodes[e[1]][0]-self.nodes[e[0]][0])**2 + (self.nodes[e[1]][1]-self.nodes[e[0]][1])**2)**0.5
            else:
    ...
    def main(self):
        ...
        for t in tqdm(range(self.Repeat), leave=False):
            #print('t=%d'%(t))
