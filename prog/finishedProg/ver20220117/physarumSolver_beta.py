...
class PhysarumSolver(object):
    ...
    def __init__(self, G, nodes, edges, nodesNum, source, sink, totalFlow, repeatNum, delta_t, mu):
        self.G = G
        self.nodes = nodes
        self.edges = edges
        self.N = nodesNum
    ...
    def SetA(self):
        ...
        for e in self.D:
            ...
            if e not in blacklist_e:
                self.G.edges[e]['C'] = self.D[e]/self.L[e]
                blacklist_e.append(revers_e)
            else:
                self.G.edges[e]['C'] = self.D[revers_e]/self.L[revers_e]
    ...
    def main(self):
        ...
        for e in nx.edges(self.G):
            # manhattan distance
            self.L[e] = ((self.nodes[e[0]][0]-self.nodes[e[1]][0])**2 + (self.nodes[e[0]][1]-self.nodes[e[1]][1])**2)**0.5
        ...
        for t in range(self.Repeat):
            ...
            #式6左辺、Dをもとに更新
            A = self.SetA()
            with open('b_result.csv', 'wt') as f:
                writer = csv.writer(f)
                writer.writerows(B)
